...
const serviceSchema=z.enum(["Website Maintenance","Web Development","Web Design"]);
const schema=z.object({email:z.string().trim().email(),name:z.string().max(120).optional(),company:z.string().max(160).optional(),service:serviceSchema.optional(),message:z.string().max(10000).optional(),source:z.string().min(2).max(120),subject:z.string().min(6).max(180),kind:z.enum(["rfp","newsletter"]),pageUrl:z.string().max(500).optional()});
...
function autoReplyText(name:string|undefined,service:string|undefined){
  return [
    `Hi ${name?.trim()||"there"},`,
    "",
...
export async function POST(req:Request){
 try{
  ...
  if(parsed.data.kind==="rfp"&&(!parsed.data.name||!parsed.data.message||!parsed.data.service))return NextResponse.json({error:"Missing required fields"},{status:400});
  ...
  const d=parsed.data; const from=process.env.SMTP_FROM||process.env.SMTP_USER; const text=[`Source page: ${d.source}`,`Page URL: ${d.pageUrl||"-"}`,`Date/time: ${new Date().toISOString()}`,`Name: ${d.name||"-"}`,`Email: ${d.email}`,`Company: ${d.company||"-"}`,`Service: ${d.service||"-"}`,`Upload: ${uploadName}`,`Message: ${d.message||"-"}`].join("\n");
  ...
  await transport.sendMail({from,to:d.email,subject:autoReplySubject(d.kind,d.source),text:autoReplyText(d.name,d.service)});
