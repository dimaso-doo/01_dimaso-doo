...
export function TrustStrip(){
 const proof=[
  ["Since 2008","Long-term website delivery and care"],
  ["Senior access","Work directly with the people responsible"],
  ["US + international","Remote collaboration across time zones"],
  ["Build + maintain","One team across the website lifecycle"],
 ];
...
export function Cases({limit,slugs}:{limit?:number;slugs?:string[]}){const studies=slugs?slugs.map(slug=>caseStudies.find(c=>c.slug===slug)).filter((c):c is (typeof caseStudies)[number]=>Boolean(c)):caseStudies;return <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:18}} className="cards-grid work-grid">{studies.slice(0,limit).map((c,i)=>{const outcome=c.result.split(". ")[0];return <article className="card case-card" key={c.name}><Link href={`/case-studies/${c.slug}`} className={`case-visual case-visual-${i+1}`} aria-label={`Read ${c.name} case study`}>{c.image&&<Image className="case-visual-image" src={c.image} alt={c.imageAlt} width={1200} height={700} sizes="(max-width: 900px) calc(100vw - 28px), 580px" loading="lazy"/>}<span className="case-logo-mark">{c.name.split(" ").map(x=>x[0]).join("").slice(0,2)}</span><span className="case-logo-name">{c.name}</span><span className="case-visual-line one"/><span className="case-visual-line two"/></Link><div className="case-content"><div className="case-meta"><span className="tag">{c.category}</span><span>{c.services}</span></div><h3><Link href={`/case-studies/${c.slug}`}>{c.name}</Link></h3><p className="muted case-summary">{c.summary}</p><div className="case-card-outcome"><span className="eyebrow">Outcome</span><p>{outcome}.</p></div><div className="case-links"><Link href={`/case-studies/${c.slug}`} className="text-link">View case study →</Link></div></div></article>})}</div>}
